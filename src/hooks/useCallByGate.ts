...
import { LocationGateData, ApiResponse, mockYearlyData } from '@/data/mock/callByGateData';

interface CallByGateParams {
  year: string;
  month: string;
  region?: string;
  page?: number;
...
export const useCallByGateData = (params: CallByGateParams) => {
  const [data, setData] = useState<LocationGateData[] | null>(null);
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.get<ApiResponse>('/api/call/by-gate', {
        params: {
          year: params.year,
          month: params.month,
      ...
      console.error('Error fetching call by gate data:', err);
